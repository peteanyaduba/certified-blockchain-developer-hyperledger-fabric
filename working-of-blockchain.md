...
1.  Client or Application (SDK): submits a proposal to endorsing peers
2.  Endorsing Peer: Execute chaincode to simulate proposal in itself (peer)
    -   Query state DB for reads
    -   Build Read/Write set
3.  Endorsing Peer: sends proposal (including Read/WriteSet) response back to Client
4.  Client (SDK): Submits transaction (includig Read/WrtieSet) to Ordering Service
...
7.  Committing Peers: Validates each transaction and commit block to blockchain
...
32. After verifying it, the endorsing peers sign the transaction proposals with their certificate 
